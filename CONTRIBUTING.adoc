...
> Note: The corresponding test files would be stored in `git-client-plugin/target/surefire-reports` directory.
>
> * `jmh.benchmark.BenchmarkRunner.txt`
> * `TEST-jmh.benchmark.BenchmarkRunner.xml`
> * `jmh.benchmark.BenchmarkRunner-output.txt`
